...
function calcDelivery(event: string, additionalRuns: number = 0, runSource: RunSource = 'bat'): DeliveryConfig {
    switch (event) {
        case 'dot': return { event, batRuns: 0, extraRuns: 0, totalRuns: 0, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'single': return { event, batRuns: 1, extraRuns: 0, totalRuns: 1, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'double': return { event, batRuns: 2, extraRuns: 0, totalRuns: 2, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'triple': return { event, batRuns: 3, extraRuns: 0, totalRuns: 3, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'four': return { event, batRuns: 4, extraRuns: 0, totalRuns: 4, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'boundary': return { event, batRuns: 4, extraRuns: 0, totalRuns: 4, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'overthrow': return { event, batRuns: 2, extraRuns: 2, totalRuns: 4, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'six': return { event, batRuns: 6, extraRuns: 0, totalRuns: 6, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'boundary_six': return { event, batRuns: 6, extraRuns: 0, totalRuns: 6, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'overthrow_six': return { event, batRuns: 3, extraRuns: 3, totalRuns: 6, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'seven': return { event, batRuns: 7, extraRuns: 0, totalRuns: 7, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bat' };
        case 'wicket': return { event, batRuns: 0, extraRuns: 0, totalRuns: additionalRuns, bowlScore: additionalRuns >= 0 ? -additionalRuns : 0, isLegal: true, isWicket: true, runSource: 'bat' };
        case 'noball': {
            const isBatRuns = runSource === 'bat';
            const batterCredit = isBatRuns ? additionalRuns : 0;
            const byeCredit = !isBatRuns ? additionalRuns : 0;
            return { event, batRuns: batterCredit, extraRuns: 1 + byeCredit, totalRuns: 1 + additionalRuns, bowlScore: 0, isLegal: true, isWicket: false, runSource };
        }
        case 'wide': return { event, batRuns: 0, extraRuns: 1 + additionalRuns, totalRuns: 1 + additionalRuns, bowlScore: 0, isLegal: false, isWicket: false, runSource: 'bye' };
        case 'bye': return { event, batRuns: 0, extraRuns: additionalRuns, totalRuns: additionalRuns, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bye' };
        case 'legbye': return { event, batRuns: 0, extraRuns: additionalRuns, totalRuns: additionalRuns, bowlScore: 0, isLegal: true, isWicket: false, runSource: 'bye' };
...
const EVENT_LABELS: Record<string, { label: string; icon: string; color: string }> = {
    ...
    boundary: { label: 'BOUNDARY', icon: '4', color: '#f97316' },
    overthrow: { label: 'OVERTHROW', icon: '4', color: '#f97316' },
    six: { label: 'SIX', icon: '6', color: '#22c55e' },
    boundary_six: { label: 'SIX', icon: '6', color: '#22c55e' },
    overthrow_six: { label: 'SIX (OVT)', icon: '6', color: '#06b6d4' },
    seven: { label: '7 RUNS', icon: '7', color: '#db2777' },
...
const FORMAT_OVERS: Record<string, number> = { '2': 2, '2 Overs': 2, '11': 11, '11 Overs': 11, 'T10': 10, 'T20': 20, 'ODI': 50, 'Test': 90 };
function getOversFromFormat(format?: string): number { return FORMAT_OVERS[format ?? ''] ?? 11; }
...
// ── TASK 1: Four Run Options Modal ────────────────────────────────────────────
function FourRunOptionsModal({ onConfirm, onCancel }: {
    onConfirm: (event: 'boundary' | 'overthrow') => void; onCancel: () => void;
}) {
    return (
        <div style={MODAL_BG}>
            <div style={{ ...MODAL_CARD, overflow: 'visible' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, borderRadius: '14px 14px 0 0', background: 'linear-gradient(90deg, transparent, #f97316, transparent)' }} />
                <ModalTitle>4 RUNS — HOW?</ModalTitle>
                <ModalSub>Select how the batter scored 4 runs</ModalSub>
                <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                    <button
                        onClick={() => onConfirm('boundary')}
                        style={{
                            flex: 1, padding: '16px 12px', background: '#f9731615', border: '1.5px solid #f97316',
                            borderRadius: 8, color: '#f97316', fontFamily: 'Orbitron', fontSize: 12, letterSpacing: 1,
                    ...
                    >
                        ...
                        <div>
                            <div>BOUNDARY</div>
                            <div style={{ fontSize: 9, color: '#f9731688', marginTop: 2 }}>4 by bat</div>
                        </div>
                    </button>
                    <button
                        onClick={() => onConfirm('overthrow')}
                        style={{
                            flex: 1, padding: '16px 12px', background: '#3b82f615', border: '1.5px solid #3b82f6',
                            borderRadius: 8, color: '#3b82f6', fontFamily: 'Orbitron', fontSize: 12, letterSpacing: 1,
                            cursor: 'pointer', fontWeight: 700, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8
                        }}
                    >
                        <span style={{ fontSize: 24 }}>🔄</span>
                        <div>
                            <div>OVERTHROW</div>
                            <div style={{ fontSize: 9, color: '#3b82f688', marginTop: 2 }}>2 runs + 2 extra</div>
                        </div>
                    </button>
                </div>
                <button onClick={onCancel} style={{ width: '100%', background: '#ef444415', border: '1px solid #ef444433', color: '#ef4444', fontFamily: 'Orbitron', fontSize: 11, letterSpacing: 1, padding: '8px 20px', borderRadius: 6, cursor: 'pointer' }}>CANCEL</button>
            </div>
        </div>
    );
}

// ── Six Run Options Modal ─────────────────────────────────────────────────────
function SixRunOptionsModal({ onConfirm, onCancel }: {
    onConfirm: (event: 'boundary_six' | 'overthrow_six') => void; onCancel: () => void;
}) {
    return (
        <div style={MODAL_BG}>
            <div style={{ ...MODAL_CARD, overflow: 'visible' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, borderRadius: '14px 14px 0 0', background: 'linear-gradient(90deg, transparent, #22c55e, transparent)' }} />
                <ModalTitle>6 RUNS — HOW?</ModalTitle>
                <ModalSub>Select how the batter scored 6 runs</ModalSub>
                <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                    <button
                        onClick={() => onConfirm('boundary_six')}
                        style={{
                            flex: 1, padding: '16px 12px', background: '#22c55e15', border: '1.5px solid #22c55e',
                            borderRadius: 8, color: '#22c55e', fontFamily: 'Orbitron', fontSize: 12, letterSpacing: 1,
                            cursor: 'pointer', fontWeight: 700, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8
                        }}
                    >
                        <span style={{ fontSize: 24 }}>🚀</span>
                        <div>
                            <div>BOUNDARY</div>
                            <div style={{ fontSize: 9, color: '#22c55e88', marginTop: 2 }}>6 over the rope</div>
                        </div>
                    </button>
                    <button
                        onClick={() => onConfirm('overthrow_six')}
                        style={{
                            flex: 1, padding: '16px 12px', background: '#06b6d415', border: '1.5px solid #06b6d4',
                            borderRadius: 8, color: '#06b6d4', fontFamily: 'Orbitron', fontSize: 12, letterSpacing: 1,
                            cursor: 'pointer', fontWeight: 700, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8
                        }}
                    >
                        <span style={{ fontSize: 24 }}>🔄</span>
                        <div>
                            <div>OVERTHROW</div>
                            <div style={{ fontSize: 9, color: '#06b6d488', marginTop: 2 }}>3 runs + 3 extra</div>
                        </div>
                    </button>
                </div>
                <button onClick={onCancel} style={{ width: '100%', background: '#ef444415', border: '1px solid #ef444433', color: '#ef4444', fontFamily: 'Orbitron', fontSize: 11, letterSpacing: 1, padding: '8px 20px', borderRadius: 6, cursor: 'pointer' }}>CANCEL</button>
...
// ── TASK 2: Wicket with Runs Modal ────────────────────────────────────────────
function WicketWithRunsModal({ onConfirm, onCancel }: {
    onConfirm: (runs: number) => void; onCancel: () => void;
}) {
    ...
    return (
        <div style={MODAL_BG}>
            <div style={{ ...MODAL_CARD, overflow: 'visible' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, borderRadius: '14px 14px 0 0', background: 'linear-gradient(90deg, transparent, #ef4444, transparent)' }} />
                <ModalTitle>WICKET — WITH RUNS?</ModalTitle>
                <ModalSub>Run-out or runs with dismissal (e.g., run-out 1, run-out 2)</ModalSub>
                <div style={{ background: '#111827', border: '1px solid #ef444433', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 11, fontFamily: 'Rajdhani', color: '#9ca3af' }}>
                    Wicket scenario (e.g., run-out on {runs} run). Batting team gets {runs} runs. Bowler loses {runs} points.
                </div>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    <button onClick={() => onConfirm(runs)} style={btnStyle('#ef444422', '#ef4444', true)}>CONFIRM W+{runs}</button>
                    <button onClick={onCancel} style={btnStyle('#37415144', '#374151', true)}>CANCEL</button>
                </div>
            </div>
        </div>
    );
        ...
        <div style={MODAL_BG}>
            <div style={{ ...MODAL_CARD, maxWidth: 480 }}>
                ...
                <div style={{ background: '#111827', border: `1px solid ${accentColor}33`, borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 11, fontFamily: 'Rajdhani', color: '#9ca3af' }}>
                    {isWide && `Batting team: 1 (wide) + ${runs} (bye) = ${1 + runs} runs. Batter: 0.`}
                    {isNoBall && runSource === 'bat' && `Batting team: 1 (NB) + ${runs} (bat) = ${1 + runs}. Batter: +${runs}.`}
                    {isNoBall && runSource !== 'bat' && `Batting team: 1 (NB) + ${runs} (${runSource}) = ${1 + runs}. Batter: 0.`}
                    {isBye && `Batting team: +${runs} bye extras. Batter: 0.`}
                    {isLegBye && `Batting team: +${runs} leg-bye extras. Batter: 0.`}
                </div>
...
// ── TASK 3: Extras Display Helper ─────────────────────────────────────────────
function ExtrasDisplay({ inn }: { inn: Innings }) {
    if (inn.extras === 0) return null;
    const extraCounts: Record<string, number> = {};
    for (const ball of inn.balls) {
        if (ball.event === 'wide') extraCounts['wide'] = (extraCounts['wide'] ?? 0) + 1;
        if (ball.event === 'noball') extraCounts['noball'] = (extraCounts['noball'] ?? 0) + 1;
        if (ball.event === 'bye') extraCounts['bye'] = (extraCounts['bye'] ?? 0) + ball.extraRuns;
        if (ball.event === 'legbye') extraCounts['legbye'] = (extraCounts['legbye'] ?? 0) + ball.extraRuns;
    }

    const parts: string[] = [];
    if (extraCounts['wide']) parts.push(`${extraCounts['wide']}w`);
    if (extraCounts['noball']) parts.push(`${extraCounts['noball']}nb`);
    if (extraCounts['bye']) parts.push(`${extraCounts['bye']}b`);
    if (extraCounts['legbye']) parts.push(`${extraCounts['legbye']}lb`);

    const display = parts.length > 0 ? parts.join(' + ') : '0';

    return (
        <div style={{ fontSize: 10, color: '#6b7280', fontFamily: 'Rajdhani', marginBottom: 10 }}>
            Extras: <span style={{ color: '#9ca3af', fontWeight: 700 }}>{inn.extras}</span> ({display})
        </div>
    );
}
                ...
                <div style={{ marginBottom: 18 }}>
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, fontFamily: 'Rajdhani' }}>
                            ...
                            <tbody>
                                {batters.map((b, i) => {
                                    const isStriker = i === inn.strikerIdx && !b.dismissed;
                                    const isNS = i === inn.nonStrikerIdx && !b.dismissed;
                ...
                <div>
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, fontFamily: 'Rajdhani' }}>
                            ...
                            <tbody>
                                {bowlers.map((b, i) => {
                                    const isCurrent = i === inn.bowlerIdx;
...
function BallDot({ b, idx }: { b: Ball; idx: number }) {
    ...
    const leftColor = b.event === 'wicket' ? '#ef4444'
        : b.event === 'six' ? '#22c55e'
            : b.event === 'boundary_six' ? '#22c55e'
                : b.event === 'overthrow_six' ? '#06b6d4'
                    : b.event === 'four' ? '#f97316'
                        : b.event === 'boundary' ? '#f97316'
                            : b.event === 'overthrow' ? '#3b82f6'
                                : b.event === 'wide' ? '#a855f7'
...
export default function BCLScoring({ scoringMatch, updateMatch, setActivePage }: {
    ...
    function applyDelivery(event: string, additionalRuns: number = 0, runSource: RunSource = 'bat') {
        ...
        setInnings(prev => {
            ...
            if (event === 'four' || event === 'boundary') bat.fours += 1;
            if (event === 'six' || event === 'boundary_six') bat.sixes += 1;
            ...
            if (event === 'four' || event === 'boundary') bwl.fours += 1;
            if (event === 'six' || event === 'boundary_six') bwl.sixes += 1;
            if (event === 'overthrow') { bwl.fours += 1; }
            if (event === 'overthrow_six') { bwl.sixes += 1; }
    ...
    function handleEvent(event: string) {
        ...
        if (event === 'four') { 
            setModal({ type: 'fourOptions' }); 
        } else if (event === 'six') { 
            setModal({ type: 'sixOptions' }); 
        } else if (event === 'wicket') { 
            setModal({ type: 'wicketRuns' }); 
        } else if (['noball', 'wide', 'bye', 'legbye'].includes(event)) { 
    ...
    const SCORING_EVENTS = ['dot', 'single', 'double', 'triple', 'four', 'six', 'seven', 'wicket', 'noball', 'wide', 'bye', 'legbye'];
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#060c14', color: '#f9fafb', fontFamily: 'Rajdhani', paddingBottom: 60 }}>
            ...
            {modal?.type === 'fourOptions' && (
                <FourRunOptionsModal
                    onConfirm={(event) => { applyDelivery(event); setModal(null); }}
                    onCancel={() => setModal(null)}
                />
            )}
            {modal?.type === 'sixOptions' && (
                <SixRunOptionsModal
                    onConfirm={(event) => { applyDelivery(event); setModal(null); }}
                    onCancel={() => setModal(null)}
                />
            )}
            {modal?.type === 'wicketRuns' && (
                <WicketWithRunsModal
                    onConfirm={(runs) => { applyDelivery('wicket', runs); setModal(null); }}
                    onCancel={() => setModal(null)}
                />
            )}
            {modal?.type === 'extra' && (
